# To compute solutions of size <n>:
# gap> construct(n);
#
# Steps:
# 1. Iterate over representatives T of conjugacy classes of Sym_n (in reverse order)
# 2. For each T, create a savilerow source file. 
#    We consider as constraints that our solution M is <= M^g for all permutations g 
#    In the centralizer that moves at most <= 3 points.
# 3. Run savilerow.
# 4. Parse the output filename to keep only those matrices which are minimal inside the orbit.
# 5. Create a GAP file with the list of matrices. 
LoadPackage("IO");
...
run := function(n, T, filename)
  ...
  create_file(n, T, Filtered(centralizer, x->NrMovedPoints(x) <= 3), Concatenation(filename, ".eprime"));
